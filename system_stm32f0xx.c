...
static void SetSysClock(void)
{
  ...
  /* --No external crystal on STM32F0 board, let's use the HSI integrated 8MHz osc-- */
  ...
  if (HSIStatus == (uint32_t)0x01)
  {
    ...
    RCC->AHBENR &= (uint32_t)((uint32_t)~(RCC_AHBENR_GPIOAEN));
    RCC->AHBENR |= (uint32_t)RCC_AHBENR_GPIOAEN; 
      
    RCC->AHBENR &= (uint32_t)((uint32_t)~(RCC_AHBENR_GPIOCEN));
    RCC->AHBENR |= (uint32_t)RCC_AHBENR_GPIOCEN; 

    /* Select PLL/2 as clock pin output */
    RCC->CFGR &= (uint32_t)((uint32_t)~(RCC_CFGR_MCO));
    RCC->CFGR |= (uint32_t)RCC_CFGR_MCO_PLL;   
